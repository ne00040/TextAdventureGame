...
class TestLocationLoader {
	...
	void testLoadLocationsValid() throws IOException {
		...
		assertTrue(forest.getAdjacentLocations().stream().anyMatch(loc -> loc.getName().equals("Village")),
				"Forest should be adjacent to Village.");
		assertTrue(forest.getAdjacentLocations().stream().anyMatch(loc -> loc.getName().equals("Cave")),
				"Forest should be adjacent to Cave.");
		...
		assertTrue(village.getAdjacentLocations().stream().anyMatch(loc -> loc.getName().equals("Forest")),
				"Village should be adjacent to Forest.");
		...
		assertTrue(cave.getAdjacentLocations().stream().anyMatch(loc -> loc.getName().equals("Forest")),
				"Cave should be adjacent to Forest.");

		assertEquals(2, forest.getAvailableActions().size(), "Forest should have 2 actions.");
		assertTrue(forest.getAvailableActions().stream().anyMatch(action -> action.getName().equals("Move")),
				"Forest should have a 'Move' action.");
		assertTrue(forest.getAvailableActions().stream().anyMatch(action -> action.getName().equals("Pick")),
				"Forest should have a 'Pick' action.");
		assertEquals(
				"Go deeper into the forest.", forest.getAvailableActions().stream()
						.filter(action -> action.getName().equals("Move")).findFirst().get().getDescription(),
				"Move action description should match.");

		assertEquals(2, village.getAvailableActions().size(), "Village should have 2 actions.");
		assertTrue(village.getAvailableActions().stream().anyMatch(action -> action.getName().equals("Talk")),
				"Village should have a 'Talk' action.");
		assertTrue(village.getAvailableActions().stream().anyMatch(action -> action.getName().equals("Trade")),
				"Village should have a 'Trade' action.");

		assertEquals(1, cave.getAvailableActions().size(), "Cave should have 1 action.");
		assertTrue(cave.getAvailableActions().stream().anyMatch(action -> action.getName().equals("Enter")),
				"Cave should have an 'Enter' action.");
	}
